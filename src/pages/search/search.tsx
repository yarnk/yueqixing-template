                ...
                <View className="b">
                  {
                    historyKeywordList.map(item => {
                      return <View className="item" onClick={() => this.onKeywordTap(item.keyword)} key={item.keyword}
                ...
                <View className="b">
                  {
                    hotKeywordList.map(item => {
                      return <View className={`item ${item.is_hot === 1 ? 'active' : ''}`} hoverClass="navigator-hover"
                        onClick={() => this.onKeywordTap(item.keyword)} key={item.id}
                      >{item.keyword}</View>;
                    })
                  }
                </View>
              ...
              keyword && <View className="shelper-list">
                {
                  Array.isArray(helpKeyword) && helpKeyword.map(item => {
                    return <View className="item" hoverClass="navigator-hover" key={item.id}
                      onClick={_ => this.onKeywordTap(item)}
                    >{item}</View>;
                  })
                }
              </View>
          ...
          searchStatus && goodsList.length && <View className="search-result">
            <View className="sort">
              <View className="sort-box">
                <View className={`item ${currentSortType == 'default' ? 'active' : ''}`}
                  onClick={() => this.openSortFilter('defaultSort')}
                >
                  <Text className="txt">综合</Text>
                </View>
                <View className={`item ${currentSortType == 'price' ? 'active' : ''}`}
                  onClick={() => this.openSortFilter('priceSort')}
                >
                  <Text className="txt">价格</Text>
                  {
                    currentSortType == 'price' && currentSortOrder == 'asc' &&
                    <AtIcon value="chevron-up" size="18" color="#666" />
                  }
                  {
                    currentSortType == 'price' && currentSortOrder == 'desc' &&
                    <AtIcon value="chevron-down" size="18" color="#666" />
                  }
                </View>
                <View className={`item ${currentSortType == 'category' ? 'active' : ''}`}
                  onClick={() => this.openSortFilter('categoryFilter')}
                >
                ...
                categoryFilter && <View className="sort-box-category">
                  {
                    Array.isArray(filterCategoryList) && filterCategoryList.map((item, index) => {
                      return <View className={`item ${item.selected ? 'active' : ''}`} key={item.id}
              ...
              <View className="b">
                {
                  goodsList && goodsList.map((item, index) => {
                    return <Navigator className={`item ${(index + 1) % 2 == 0 ? 'item-b' : ''}`}
                      url="/pages/goods/goods?id={item.id}" key={item.id}
                    >
